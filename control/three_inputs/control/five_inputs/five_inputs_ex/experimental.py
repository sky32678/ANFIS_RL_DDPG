...
def train_anfis_with(model, data, optimizer, criterion,
    ...
    #    print(model.layer['fuzzify'].varmfs['theta_lookahead'].mfdefs['mf2'].b.requires_grad)
...
def train_anfis(model, data, epochs=500, show_plots=False):
    ...
    #if show_plots:
    #    plot_all_mfs(model, x)
    optimizer = torch.optim.SGD(model.parameters(), lr=1e-5/1.05, momentum=0.99)
