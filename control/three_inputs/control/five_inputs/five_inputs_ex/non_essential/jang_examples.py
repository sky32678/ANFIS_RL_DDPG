...
def ex3_model(mfnum=7):
    ...
    outvars = ['y']
    model = anfis.AnfisNet('Jang\'s example 3', invardefs, outvars,False)
    return model
...
def ex4_model():
    ...
    '''
    invardefs = [
            ('xm18', make_bell_mfs(0.444045, 2, [0.425606, 1.313696,0.8,0.7777])),
            ('xm12', make_bell_mfs(0.444045, 2, [0.425606, 1.313696,0.732,1.2323])),
            ('xm6',  make_bell_mfs(0.444045, 2, [0.425606, 1.313696,0.6,1.11])),
            ('x',    make_bell_mfs(0.444045, 2, [0.425606, 1.313696,0.92,0.421])),
            ]
    outvars = ['xp6']
    ###add False at the end if you don't want hybrid
    #######################################################################3
    #model = anfis.AnfisNet('Jang\'s example 4', invardefs, outvars)
    model = anfis.AnfisNet('Jang\'s example 4', invardefs, outvars,False)
    return model
